...
export const getUserProjects = asyncHandler(async (req, res) => {
    ...
    res.status(200).json(new ApiSuccessResponse(true, 200, "Project found", projects));
});
export const updateProject = asyncHandler(async (req, res) => {
    ...
    if (req.project.owner.toString() !== userId.toString()) {
        throw new ApiErrorResponse(400, "Only owner can change project owner");
    }
    ...
    if (ownerEmail) {
        ...
        if (!ownerUser) {
            throw new ApiErrorResponse(404, "New Owner not founnd by given email");
        }
    ...
    res.status(200).json(new ApiSuccessResponse(true, 200, "Project updated successfully", updatedproject));
});
export const deleteProject = asyncHandler(async (req, res) => {
    ...
    const projectId = req.project?._id;
    if (req.project.owner.toString() !== userId.toString()) {
        throw new ApiErrorResponse(400, "Only owner can delete the project");
    }
    ...
    if (!deletedProject) {
        throw new ApiErrorResponse(404, "Project not found or you're not authorized to delete it");
    }
    const deletedProjectNotes = await Note.deleteMany({
        project: deletedProject._id,
    });
    if (!deletedProjectNotes) {
        throw new ApiErrorResponse(404, "project have no notes or error deleting");
    }
    res.status(200).json(new ApiSuccessResponse(true, 200, "Project deleted successfully", {
        deletedProject,
        deletedProjectNotes,
    }));
});
export const addMemberToProject = asyncHandler(async (req, res) => {
    ...
    if (project.owner.toString() !== userId.toString()) {
        throw new ApiErrorResponse(400, "Only owner can add members the project");
    }
    if (!memberEmail) {
    ...
    if (project.members?.includes(memberUser._id)) {
        throw new ApiErrorResponse(400, "User is already a member of the project");
    }
    project.members?.push(memberUser._id);
    await project.save();
    await project.populate([{ path: "owner" }, { path: "members" }]);
    res.status(200).json(new ApiSuccessResponse(true, 200, "Member added to project successfully", project));
});
export const removeMemberFromProject = asyncHandler(async (req, res) => {
    ...
    const { memberEmail } = req.body;
    if (project.owner.toString() !== userId.toString()) {
        throw new ApiErrorResponse(400, "Only owner can remove members from the project");
    }
    if (!memberEmail) {
        throw new ApiErrorResponse(400, "Member email is required");
    }
    ...
    if (!project.members?.includes(memberUser._id)) {
        throw new ApiErrorResponse(400, "User is not a member of the project");
    }
    project.members = project.members?.filter((memberId) => memberId.toString() !== memberUser._id.toString());
    await project.save();
    await project.populate([{ path: "owner" }, { path: "members" }]);
    res.status(200).json(new ApiSuccessResponse(true, 200, "Member removed from project successfully", project));
});
