...
export const getNote = asyncHandler(async (req, res) => {
    ...
    return res
        .status(400)
        .json(new ApiSuccessResponse(true, 400, "Note retrieved successfully", note));
});
export const getNotes = asyncHandler(async (req, res) => {
    ...
    return res
        .status(200)
        .json(new ApiSuccessResponse(true, 200, "Notes are fetched successfully"));
});
export const updateNote = asyncHandler(async (req, res) => {
    const { title, content } = req.body;
    const { noteId } = req.params;
    const user_id = req.user?._id;
    const updatedNote = await Note.findOneAndUpdate({ _id: noteId, user: user_id }, { title, content }, { new: true });
    if (!updatedNote) {
        throw new ApiErrorResponse(404, "Unable to update the note");
    }
    return res
        .status(200)
        .json(new ApiSuccessResponse(true, 200, "note updated successfully", updatedNote));
});
...
export const archiveNote = asyncHandler(async (req, res) => {
    ...
    return res
        .status(200)
        .json(new ApiSuccessResponse(true, 200, "Note is archived successfully", note));
});
export const pinNote = asyncHandler(async (req, res) => {
    ...
    return res
        .status(200)
        .json(new ApiSuccessResponse(true, 200, "Note is pinned successfully", note));
});
export const searchNote = asyncHandler(async (req, res) => {
    ...
    return res
        .status(200)
        .json(new ApiSuccessResponse(true, 200, "Searching successful", results));
});
